...
from poly_mask_eval_2 import poly_mask_eval
import shapely
...
#input_dir = "path/to/datasets/UTKFace/"
input_dir = "../../datasets/UTKFace-curated/"
...
#images = random.sample(images, 20000)

rhytide_polygon_index = {
    'forehead': [18, 19, 20, 21, 24, 25, 26, 27], # removed "1, " from beginning
...
for row in images:
    ...
    if(file.endswith(".jpg")):
        ...
        if len(faces)>0:
            ...
            if len(faces)>0:
                ...
                for key in rhytide_polygon_index:
                    ...
                    if key=='forehead':
                        # for the forehead, we extend the polygon up to the bounding box edge from the widest/narrowest points of the face
                        (x,y,w,d) = faces[0]
                        #polygon = np.append(polygon, [[x+w,y], [x,y]], axis=0)
                        polygon = np.append(polygon, [[np.max(polygon,axis=0)[0],y], [np.min(polygon,axis=0)[0],y]], axis=0)
                    # next three lines scale down the polygon:
                    p = shapely.geometry.Polygon(polygon)
                    p_scaled = shapely.affinity.scale(p, xfact=0.8, yfact=0.6)
                    polygon = np.asarray(p_scaled.exterior.coords)
                    polygon_centroid = np.average(polygon, axis=0)
                    #get weighted edge count in polygon:
                    edges = poly_mask_eval(cropped, np.int32([polygon]), blockSize = 5, weightedMean = 8, name=key)
                    total += edges
                ...
                # uncomment these lines to show/save the (annotated) image
                #plt.axis("off")
                ...
                this_row.append(str(total))
                print(','.join(this_row))
                num_faces += 1
                #if num_faces > 5: break
        ...
        # lines beginning "XXXXXXXX" can be removed from CSV output (use `grep -v "XXXXXXX" file.csv` or similar)
